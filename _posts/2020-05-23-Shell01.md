...
```shell
[atguigu@hadoop102 datas]$ awk '/^$/{print NR}' sed.txt #^匹配行开始，$匹配行结束，如果中间没有其他内容，说明是个空行
5
```
...
```shell
[atguigu@hadoop102 datas]$ awk -F: '/^root/{print $1","$7}' passwd #正则表达式以什么开始用^,以什么结束用$
root,/bin/bash#$1 第一列，$7 第7列
```
...
```shell
[atguigu@hadoop102 datas]$ awk -F : 'BEGIN{print "user, shell"} {print $1","$7} END{print "dahaige,/bin/zuishuai"}' passwd #BEGIN代表在所有行前面，END代表在所有行后面
user, shell
root,/bin/bash
bin,/sbin/nologin
。。。
...
```shell
[atguigu@hadoop102 datas]$ awk -v i=1 -F: '{print $3+i}' passwd # -v 赋值一个用户定义变量
1
2
3
4
...
# grep -r “shen” /home | awk -F : '{print $1}'
/home/atguigu/datas/sed.txt
/home/atguigu/datas/cut.txt
# grep -r “shen” /home | awk -F : '{print $1}' | awk -F "/" '{print $4}'
cut.txt
sed.txt
# grep -r “shen” /home | awk -F : '{print $1}' | awk -F "/" '{print $NF}'
cut.txt
sed.txt
```